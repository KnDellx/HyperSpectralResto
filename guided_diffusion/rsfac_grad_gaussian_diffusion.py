...
class GaussianDiffusion:
    ...
    def p_sample(
        ...
        c1 = (
                eta * (
                    (1 - noise_level / noise_level_next) * (1 - noise_level_next) / (1 - noise_level + eps)).sqrt()
        )
        c2 = ((1 - noise_level_next) - c1 ** 2).sqrt()
        xt_next = noise_level_next.sqrt() * pred_xstart + c1 * th.randn_like(x) + c2 * model_output

        out = {"sample": xt_next, "pred_xstart": pred_xstart, "xhat": xhat}
    ...
    def q_sample(
        ...
        if isinstance(x_start, np.ndarray):
            x_start = th.from_numpy(x_start)

        # 获取 timesteps 所在的设备
        device = t.device
        # 将 x_start 移动到 timesteps 所在的设备
        ...
        alphas_cumprod_tensor = th.from_numpy(self.alphas_cumprod).to(device)

        # 进行计算
        sqrt_alphas_cumprod = th.sqrt(alphas_cumprod_tensor)
        sqrt_one_minus_alphas_cumprod = th.sqrt(1 - alphas_cumprod_tensor)

        part1 = _extract_into_tensor(sqrt_alphas_cumprod, t, x_start.shape) * x_start
        part2 = _extract_into_tensor(sqrt_one_minus_alphas_cumprod, t, x_start.shape) * noise

        return part1 + part2
        # return (
        #     _extract_into_tensor(np.sqrt(self.alphas_cumprod), t, x_start.shape) * x_start
        #     + _extract_into_tensor(np.sqrt(1-self.alphas_cumprod), t, x_start.shape)
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    #print("error")
    # print(timesteps.device) # cuda:0
    # print(th.from_numpy(arr))
