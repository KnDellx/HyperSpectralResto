...
class GaussianDiffusion:
    ...
    def unmixing(
        ...
        for repeat in range(K):
            W0, _, _= vca(input_hsi.T, R)  # 初始化endmember和abundance
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():  # 不计算梯度
                ...
                if measurement is not None:
                    g1, H, _ = measurement(out_W["pred_xstart"], model_kwargs["ref_img"],
                ...
                yield out_W
                img_W = out_W["sample"]
            _H = H.cpu().detach().numpy()[:, 0]
            _W = img_W.cpu().detach().numpy()[:, 0]
            _W = (_W + 1)/2

            _psnr = PSNR(gt, _H@_W, data_range=1)
        ...
        print("SSIM", compare_ssim(gt, _H@_W, data_range=1))
...
def double_sample_loop_progressive(
    ...
    if W0 is not None: 
        img_W = diffusion_W.q_sample(W0 * 2 - 1, th.tensor([t0] * W0.shape[0], device=device), img_W)
    if H0 is not None:
        img_H = diffusion_H.q_sample(H0 * 2 - 1, th.tensor([t0] * H0.shape[0], device=device), img_H)
    ...
    for iteration, (i,j) in enumerate(zip(indices, indices_next)):
        ...
        with th.no_grad():
            ...
            out_H = diffusion_H.p_sample(
                model_H,
                img_H,  # 形状为(Bb, Cc , Hh, Ww ), B是batch_size
                t,
                t_next,
                clip_denoised=True,
                denoised_fn=None,  
        ...
        _psnr = PSNR(gt, _H.T@_W, data_range=1)  # gt 256*256, 124
        psnr_list.append(_psnr)
